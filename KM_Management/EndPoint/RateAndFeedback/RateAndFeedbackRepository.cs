...
public class RateAndFeedbackRepository : IRateAndFeedbackRepository
{
    ...
    public async Task<IEnumerable<EntityExcelRateAndFeedback?>> GetExcelRateAndFeedbackAsync(FilterExcelRateAndFeedback? filter, CancellationToken cancellationToken)
    {
        await using var connection = await _connection.CreateConnectionAsync();
        var rating = string.Join(",", filter?.Rating);
        var storeProcedureName = "[dbo].[Get_Export_User_Feedback_By_Filter]";
        var filterFeedback = new
        {
            Filter_Date = filter?.Filter_Date,
            Start_Date = filter?.Start_Date,
        ...
        var command = new CommandDefinition(storeProcedureName, filterFeedback, commandType: System.Data.CommandType.StoredProcedure, cancellationToken: cancellationToken);
        var result = await connection.QueryAsync<EntityExcelRateAndFeedback?>(command);
        return result;
    }
...
public interface IRateAndFeedbackRepository
{
    ...
    Task<IEnumerable<EntityExcelRateAndFeedback?>> GetExcelRateAndFeedbackAsync(FilterExcelRateAndFeedback? filter, CancellationToken cancellationToken);
    // Sync Fn
}
