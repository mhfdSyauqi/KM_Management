...
const firstUid = ref(storeCategories.getFirstUid(firstCategory.value))
...
const fetchSecondLayer = async (is_Active) => {
  try {
...
const addNewCategory = async (name, is_Active) => {
  try {
    ...
    newCategory.value.Layer = 2
    newCategory.value.Is_Active = is_Active
    newCategory.value.Uid_Reference = firstUid.value
...
const navigateToSecondUid = (name, uid, is_active) => {
  storeCategories.setSelectedSecondUid(name, uid, is_active)
}
...
const checkBoxChange = () => {
  if (isActiveYesToggle.value == true && isActiveNoToggle.value == true) {
    fetchSecondLayer(null) // Both checkboxes are selected, get all categories
  } else if (isActiveYesToggle.value == true) {
    fetchSecondLayer(true) // Only the "Yes" checkbox is selected, get inactive categories
  } else if (isActiveNoToggle.value == true) {
    fetchSecondLayer(false) // Only the "No" checkbox is selected, get active categories
  } else {
    // secondLayer.value = []
    fetchSecondLayer(null)
  }
}

// const groupedSecondLayer = computed(() => {
//   // Group items by the first letter of their names
//   const groups = {}
//   for (const second of secondLayer.value) {
//     const firstLetter = getCategorySecondtLetter(second.name)
//     if (!groups[firstLetter]) {
//       groups[firstLetter] = []
//     }
//     groups[firstLetter].push(second)
//   }
...
<template>
  ...
  <div class="w-full rounded-lg bg-white min-h-[25%] max-h-[96%] box-border flex flex-col">
    <div class="flex gap-2.5 justify-between border-b-2 py-5 px-8">
      ...
      <div class="flex items-end space-x-2">
        ...
        <button
          ...
          :class="{ 'bg-gray-300': secondLayer.length == 0 }"
        >
          Export to Excel
        </button>
        ...
        <PrimaryButton @click="openCreateModal" v-if="firstActive === true"> Create </PrimaryButton>
    ...
    <!-- <div class="overflow-y-auto max-h-[80%] grid grid-cols-2 md:grid-cols-3 gap-8 p-10">
          ...
          <div class="flex items-center pb-1">
            ...
            <RouterLink
              ...
              :to="{
                ...
                params: { secondLayer: firstCategory, thirdLayer: item.name }
              }"
              @click="navigateToSecondUid(item.name, item.uid, item.is_active)"
            >
              <span
                :class="{
      ...
      <div v-show="groupedSecondLayer.length === 0">
    ...
    <div class="overflow-y-auto max-h-[80%] grid grid-cols-2 md:grid-cols-3 gap-5 p-10">
      <div v-for="(subgroup, groupIndex) in groupedSecondLayer" :key="groupIndex">
        <div
                ...
                <span
                  :class="{
                    ...
                    ' mr-2 line-through text-gray-500 hover:font-semibold hover:text-[#2c7b4b]':
                      !item.is_active,
                    ' mr-2  text-gray-500 bg-yellow-300 pl-2 pr-2 hover:font-semibold rounded-2xl drop-shadow-2xl hover:text-slate-500':
                      hightLightUid == item.uid
                  }"
                >
                  {{ item.name }}
      ...
      <div v-show="secondLayer.length === 0">
        <div class="relative bg-[#eeeeee] w-[100%] pl-6 pb-4 pt-4 rounded-tr-3xl rounded-bl-3xl">
          <span class="italic mr-2 text-gray-500">Data Not Available</span>
        </div>
      </div>
      ...
      <div class="mb-4">
        ...
        <div class="pt-2 pb-2">
          ...
          <input
            ...
            class="w-full px-3 py-2 border rounded-xl focus:outline focus:outline-[#2c7b4b]"
          />
          <div v-if="errorAddCategory != ''" class="relative text-red-500 mt-2 ml-2">
            <div>
              {{ errorAddCategory }}
            </div>
          </div>
      ...
      <div class="mb-4">
        ...
        <div class="pt-2 pb-2">
          ...
          <input
            ...
            class="w-full px-3 py-2 border rounded-xl focus:outline focus:outline-[#2c7b4b]"
          />
          <div v-if="errorUpdateCategory.length != ''" class="relative text-red-500 mt-2 ml-2">
            <div>
              {{ errorUpdateCategory }}
            </div>
          </div>
