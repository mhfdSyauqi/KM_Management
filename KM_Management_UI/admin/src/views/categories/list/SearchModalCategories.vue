  ...
  try {
    ...
    for (const item of firstLayer.value) {
        ...
        for (const secondLayerItem of item.secondLayer) {
          secondLayerItem.thirdLayer = await fetchThirdLayer(
            secondLayerItem.uid,
            3,
            isActive
          );
...
<template>
    ...
    >
      ...
      <!-- Modal content -->
      <div
        class="bg-white pl-8 pr-8 rounded-2xl z-50 w-auto min-w-auto min-h-auto"
        @click.stop
      >
      ...
      >
        <div class="mb-4">
          <div class="pb-4">
            <span v-if="searchCategories.length > 0" class="text-orange-500">
              Layer 1
            </span>
          </div>
          ...
          <div class="pb-4 pt-4">
            <span v-if="secondResult.length > 0" class="text-orange-500"
              >Layer 2</span
            >
          </div>
          <div
            v-for="(secondLayer, secondIndex) in secondResult"
            :key="secondIndex"
          >

          <RouterLink :to="{ name: 'categories-second-layer', params: { secondLayer: secondLayer.first_parent_name } }"
               @click="
          ...
          <div class="pb-4 pt-4">
            <span v-if="thirdResult.length > 0" class="text-orange-500"
              >Layer 3</span
            >
          </div>
          <div v-for="(thirdLayer, thirdIndex) in thirdResult" :key="thirdIndex">
            
            <RouterLink :to="{ name: 'categories-third-layer', 
              params: {secondLayer:thirdLayer.first_parent_name ,
      ...
      <div
        v-else-if="
          searchQueryCategory.length > 0 && searchCategories.length === 0
        "
