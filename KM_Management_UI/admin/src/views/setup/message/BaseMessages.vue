...
import {
  active_message, GetActiveMessage
} from '@/components/pages/setup/useMessage.js'
...
const fetchOpening = async () => {
  ...
  isDropDownOpeningOpened.value = Array(openingMessage.value.length).fill(
      false
    );
  editModeOpening.value = Array(openingMessage.value.length).fill(false);
  updateTextOpening.value = openingMessage.value.map(
      (opening) => opening.contents
    );
};
...
const fetchLayerOne = async () => {
  ...
  isDropDownLayerOneOpened.value = Array(layerOneMessage.value.length).fill(
      false
    );
  editModeLayerOne.value = Array(layerOneMessage.value.length).fill(false);
  updateTextLayerOne.value = layerOneMessage.value.map(
      (layerOne) => layerOne.contents
    );
};
...
const fetchLayerTwo = async () => {
  ...
  isDropDownLayerTwoOpened.value = Array(layerTwoMessage.value.length).fill(
      false
    );
  editModeLayerTwo.value = Array(layerTwoMessage.value.length).fill(false);
  updateTextLayerTwo.value = layerTwoMessage.value.map(
      (layerTwo) => layerTwo.contents
    );
};
...
const fetchSolved = async () => {
  ...
  isDropDownSolvedOpened.value = Array(solvedMessage.value.length).fill(
      false
    );
  editModeSolved.value = Array(solvedMessage.value.length).fill(false);
  updateTextSolved.value = solvedMessage.value.map(
      (solved) => solved.contents
    );
};
...
const fetchNotFound = async () => {
  ...
  isDropDownNotFoundOpened.value = Array(notFoundMessage.value.length).fill(
    false
  );
  editModeNotFound.value = Array(notFoundMessage.value.length).fill(false);
  updateTextNotFound.value = notFoundMessage.value.map(
...
const fetchReasked = async () => {
  ...
  isDropDownReAskedOpened.value = Array(reAskedMessage.value.length).fill(
    false
  );
  editModeReAsked.value = Array(reAskedMessage.value.length).fill(false);
  updateTextReAsked.value = reAskedMessage.value.map(
    (reAsked) => reAsked.contents
  );
};
...
const fetchSuggestion = async () => {
  ...
  isDropDownSuggestionOpened.value = Array(suggestionMessage.value.length).fill(
    false
  );
  editModeSuggestion.value = Array(suggestionMessage.value.length).fill(false);
  updateTextSuggestion.value = suggestionMessage.value.map(
    (suggestion) => suggestion.contents
  );
};
...
const fetchIdle = async () => {
  ...
  isDropDownIdleOpened.value = Array(idleMessage.value.length).fill(
    false
  );
  editModeIdle.value = Array(idleMessage.value.length).fill(false);
  updateTextIdle.value = idleMessage.value.map(
    (idle) => idle.contents
  );
};
...
const fetchClosing = async () => {
  ...
  isDropDownClosingOpened.value = Array(closingMessage.value.length).fill(
    false
  );
  editModeClosing.value = Array(closingMessage.value.length).fill(false);
  updateTextClosing.value = closingMessage.value.map(
    (closing) => closing.contents
  );
};
...
const fetchFeedBack = async () => {
  ...
  isDropDownFeedbackOpened.value = Array(feedbackMessage.value.length).fill(
    false
  );
  editModeFeedback.value = Array(feedbackMessage.value.length).fill(false);
  updateTextFeedback.value = feedbackMessage.value.map(
    (feedback) => feedback.contents
  );
};
...
const fetchMailSended = async () => {
  ...
  isDropDownMailSendedOpened.value = Array(mailSendedMessage.value.length).fill(
    false
  );
  editModeMailSended.value = Array(mailSendedMessage.value.length).fill(false);
  updateTextMailSended.value = mailSendedMessage.value.map(
    (mailSended) => mailSended.contents
  );
};
...
<template>
  <div class="flex items-center align-middle gap-3 mb-3 bg-tea">
    ...
    <span> > </span>
    <p class="text-sm text-orange-400">Messages</p>
  </div>

  <div class="w-full rounded-lg bg-white min-h-[25%] max-h-[96%] box-border flex flex-col ">
    <div class="pl-10 pb-10 pr-10">
        <div class="sticky top-0 max-w-full h-50 bg-white flex">
          <div class="w-full">
            <div class="flex items-start space-x-2 pt-8 pb-8">
              <h1>
